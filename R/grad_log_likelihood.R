grad_ZIP_R <- function(params, X, Z, Y,
  ...
  if (missing(offsetz)) offsetz <- rep(1,nrow(Z))
  ...
  dens0 <- muz * (1 - as.numeric(!Y0)) + exp(log(1 - muz) + 
                                              clogdens0)
  
  wres_count <- ifelse(!Y0, Y - mu, -exp(-log(dens0) + 
                                          log(1 - muz) + clogdens0 + log(mu)))
  
  wres_zero <- ifelse(!Y0, -1/(1 - muz) * linkobj$mu.eta(etaz), 
