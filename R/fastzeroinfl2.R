...
fastzeroinfl2 <- function(formula, data, subset, na.action, weights, offset, 
                          ...
                          model = TRUE, y = TRUE, x = FALSE, ...) 
{
  ...
  if (is.null(start)) {
    ...
    if (control$EM & dist == "poisson") {
      ...
      probi[Y1] <- 0
      ll_new <- loglikfun(c(start$count, start$zero), x = X,
                          z = Z, y = Y, weights = weights,
                          offsetx = offsetx,
                          offsetz = offsetz,
                          link = link)
      ll_old <- 2 * ll_new
    ...
    if (control$EM & dist == "negbin") {
      ...
      probi[Y1] <- 0
      ll_new <- loglikfun(c(start$count, start$zero,log(start$theta)), x = X,
                          z = Z, y = Y, weights = weights,
                          offsetx = offsetx,
                          offsetz = offsetz,
                          link = link)
